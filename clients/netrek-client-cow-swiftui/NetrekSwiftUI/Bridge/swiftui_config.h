/* swiftui_config.h - config.h for macOS/arm64 SwiftUI+CoreGraphics build
 *
 * Replaces the autoconf-generated config.h for building COW with
 * CoreGraphics rendering + SwiftUI frontend instead of X11 or SDL2.
 */
...
#define XTREKRC_HELP
#define TOOLS
#undef  SOUND            /* Sound stubbed out for Phase 1 */
#define HOCKEY_LINES
#define BEEPLITE
...
/* CoreGraphics/SwiftUI backend - not X11, Win32, or SDL2 */
#undef  HAVE_X11
#undef  HAVE_WIN32
#undef  HAVE_XPM
...
/* SwiftUI wrapper: cap select() timeouts and pump events before blocking.
 * Events arrive from the SwiftUI event queue (not from a socket FD),
 * so we must pump before every select and keep timeouts short. */
#include <sys/select.h>
#include <sys/time.h>
extern void swiftui_pump_events(void);  /* defined in swiftuiwindow.c */
static inline int swiftui_select(int nfds, fd_set *r, fd_set *w, fd_set *e,
                                  struct timeval *tv) {
    swiftui_pump_events();
